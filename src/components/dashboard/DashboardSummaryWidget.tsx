...
/**
 * The dashboard, docked to the right of the screen — opened by "show dashboard
 * summary" and closed by "close dashboard summary".
 *
 * It reads the same data as the Dashboard page (usePatientOverview) so the two
 * can never disagree, and shows it as one scrollable column: the numbers, what
...
export function DashboardSummaryWidget() {
  ...
  const close = () => dispatch(uiActions.setDashboardSummaryOpen(false));
  ...
  const stats: Array<{ label: string; value: string | number; hint: string; tone: string; path: string }> = [
    { label: 'Active meds', value: highlights.activeMedications, hint: `${counts.medication} in total`, tone: 'primary', path: '/medications' },
    { label: 'Active problems', value: highlights.activeDiagnoses, hint: `${counts.diagnosis} in total`, tone: 'info', path: '/diagnoses' },
    { label: 'Open tasks', value: highlights.openTasks, hint: highlights.overdueTasks ? `${highlights.overdueTasks} overdue` : 'None overdue', tone: highlights.overdueTasks ? 'error' : 'success', path: '/tasks' },
    { label: 'Recalls due', value: highlights.dueRecalls, hint: `${counts.recall} in total`, tone: highlights.dueRecalls ? 'warning' : 'neutral', path: '/recalls' },
  ];
      ...
      <header className="dash-dock-head">
        ...
        <Tooltip title="Read it aloud">
          <Button
            ...
            aria-label="Read the dashboard summary aloud"
            icon={<Volume2 size={15} />}
            onClick={() => narrative && speak(narrative.text)}
          />
        </Tooltip>
        <Tooltip title='Close — or say "close dashboard summary"'>
