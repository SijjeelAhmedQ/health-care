    ...
    <>
      <section className="patient-banner" aria-label={`Selected patient: ${patient.fullName}`}>
        <div className="patient-banner-identity">
          <Avatar name={patient.fullName} size={46} />
          <div className="patient-banner-who">
            ...
            <div className="patient-banner-demographics">
              <span><strong>{patient.mrn}</strong></span>
              <span>{patient.age} years · {patient.gender}</span>
              <span>DOB {dayjs(patient.dateOfBirth).format('D MMM YYYY')}</span>
              <span>Blood group {patient.bloodGroup}</span>
            </div>
            <div className="patient-banner-contacts">
              <span><Phone size={13} aria-hidden /> {patient.phone}</span>
              <span><Mail size={13} aria-hidden /> {patient.email}</span>
              <span><UserRound size={13} aria-hidden /> {patient.primaryProviderName}</span>
            </div>
          </div>
        ...
        <ul className="patient-banner-stats">
          {stats.map((s) => (
            <li key={s.key}>
